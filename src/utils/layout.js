import { SelectMenu, FontAwesomeIcon } from "@trops/dash-react";
import {
...
export function renderGridLayout(gridLayout, args) {
  ...
  for (var i = 1; i < gridLayout.rows + 1; i++) {
    for (var j = 1; j < gridLayout.cols + 1; j++) {
      ...
      const cmpIdToRender =
        cellNumber in gridLayout ? gridLayout[cellNumber]["component"] : null;
  ...
  return (
    <div
      className={`grid grid-cols-${gridLayout["cols"] || 1} grid-rows-${
        gridLayout["rows"] || 1
      } gap-4 h-full w-full`}
    >
      {gridContents}
...
export function renderGridLayoutFlow(gridLayout, args) {
  try {
    const { item, onClickEmptyCell, onSelectWidgetForCell, workspace } = args;
  ...
  try {
    ...
    return (
      <div
        ...
        onClick={() =>
          onClickEmptyCell && onClickEmptyCell(cellNumber, cellData, workspace)
        }
      >
  ...
  return (
    componentToRender && (
      <div
        ...
        onClick={() => console.log("CLICKED CELL ", cellNumber, cellData)}
      >
        <div className="flex flex-col w-full h-full justify-center items-center">
          <LayoutBuilderGridItem
            key={`grid-item-${uuid}-${previewMode === true ? "view" : "edit"}`}
            uuid={uuid}
            item={componentToRender}
            layout={layout}
  ...
  try {
    ...
    return (
      ...
      currentWorkspace["layout"]
        ...
        .map((child) => {
          const childLayout = LayoutModel(child, currentWorkspace["layout"]);
          ...
          const hasChildren = "hasChildren" in child ? child["hasChildren"] : 0;
...
export function renderComponent(component, id, params = {}, children = null) {
  try {
    if (component) {
      ...
      return WidgetFactory.render(component, `widget-${id}`, params, children);
    } else {
      return null;
    }
  } catch (e) {
...
export function addItemToItemLayout(
  ...
  if (itemToAdd) {
    console.log("[addItemToItemLayout] itemToAdd:", itemToAdd);
    console.log(
      "[addItemToItemLayout] itemToAdd.component:",
      itemToAdd["component"],
    );

    const newItem = LayoutModel(itemToAdd, tempLayout);
    console.log("[addItemToItemLayout] newItem after LayoutModel:", newItem);
    ...
    console.log(
      "[addItemToItemLayout] newItem after setting component:",
      newItem,
    );
    console.log(
      "[addItemToItemLayout] newItem.component:",
      newItem["component"],
    );
    ...
    if (parentWorkspaceType !== null) {
      const options =
        ...
        Object.keys(componentMap)
          ...
          .filter((c) =>
            workspaceType !== null
              ? componentMap[c]["workspace"] === parentWorkspaceType
              : true,
          )
...
export function getWorkspacesForWorkspace(workspaceItem, searchTerm = "") {
  ...
  const options =
    canAddChildren === true && workspaceItem["parentWorkspaceName"] === "layout"
      ? Object.keys(componentMap)
          .sort()
          .filter((i) =>
            searchTerm !== "" ? i.toLowerCase().includes(searchTerm) : true,
          )
          .filter(
            (c) =>
          ...
          .map((w) => componentMap[w] /*renderMenuItem("workspace", w) */)
      : Object.keys(componentMap)
          .sort()
          .filter((i) =>
            searchTerm !== "" ? i.toLowerCase().includes(searchTerm) : true,
...
export function getLayoutItemForWorkspace(item, workspace, parentItem = null) {
  try {
    ...
    const layoutModel = LayoutModel(item, workspace["layout"], workspace["id"]);
