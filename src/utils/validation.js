  ...
  try {
    ...
    if (!targetComponent) {
      return {
        valid: false,
        error:
          "No target component specified. Widgets must be placed in a grid cell.",
      };
    }
    ...
    if (targetComponent.type === "widget") {
      return {
        valid: false,
        error:
          "Cannot place widget inside another widget. Widgets can only be placed in grid cells.",
      };
    }
    ...
    if (!isGridContainer && !isValidContainerType) {
      return {
        valid: false,
        error:
          "Target component is not a valid container. Widgets must be placed in grid cells.",
      };
    }
  ...
  try {
    ...
    if (!targetComponent) {
      return {
        valid: false,
        error:
          "No target component specified. Grids must be placed in a grid cell.",
      };
    }
    ...
    if (targetComponent.type === "widget") {
      return {
        valid: false,
        error:
          "Cannot place grid inside a widget. Grids can only be placed in grid cells.",
      };
    }
    ...
    const isContainerComponent = ["LayoutGridContainer", "Container"].includes(
      gridToPlace.component,
    );
    ...
    if (!hasGridProperty && !isContainerComponent) {
      return {
        valid: false,
        error:
          "Grid component does not have a grid property or valid component type.",
      };
    }
