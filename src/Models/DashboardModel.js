...
export class DashboardModel {
  ...
  setParentForLayoutItem(parentId, layoutItem) {
    try {
      ...
      layoutItem.parentWorkspaceName = layoutItem.parentWorkspace.workspace;
  ...
  compatibleWorkspaces(layoutItem) {
    return this.layout.filter(
      (item) =>
        item.type === "workspace" && item.workspace === layoutItem.workspace,
    );
  }
  ...
  layoutItemIsCompatible(layoutItemToCheck, layoutItem) {
    return (
      this.layoutItemHasWorkspaceAsChild(layoutItemToCheck, layoutItem).length >
      0
    );
  }
  ...
  findCompatibleWorkspaceComponent(layoutItem) {
    const compatibleWorkspaceComponent = ComponentManager.getWorkspaceByName(
      layoutItem.workspace,
    );
  ...
  addWorkspaceForWidget(toLayoutItem, layoutItemToAdd) {
    try {
      if (layoutItemToAdd.type === "widget") {
        ...
        const compatibleWorkspaceChildren = this.layoutItemHasWorkspaceAsChild(
          toLayoutItem,
          layoutItemToAdd,
        );
        ...
        if (compatibleWorkspaceChildren.length === 0) {
          // we have to conjur up a Compatible workspace based on the item specified
          const tempWorkspaceComponent = ComponentManager.getWorkspaceByName(
            workspaceRequiredName,
          );
