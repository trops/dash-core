...
export const PresetGallery = ({
  ...
  return (
    ...
    >
      <span className="text-sm font-semibold opacity-50">Choose a Preset</span>
...
export const ColorHarmonyPicker = ({ onGenerate, inline = false }) => {
  ...
  return (
    ...
    >
      ...
      {!inline && (
        <Button title="Generate Theme" onClick={handleGenerate} size="sm" />
      )}
    </div>
  );
};
  ...
  return (
    ...
    >
      <div className="flex flex-row items-center gap-2">
        <FontAwesomeIcon
          icon={icon}
          className={`h-3.5 w-3.5 ${selected ? "text-blue-400" : "opacity-50"}`}
        />
