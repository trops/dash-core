...
const COMPONENT_GROUPS = {
  ...
  Toggles: [themeObjects.TOGGLE, themeObjects.TOGGLE_2, themeObjects.TOGGLE_3],
  Overlays: [
    themeObjects.DRAWER,
    ...
    <LayoutContainer direction="col" scrollable={true}>
      <div className="flex flex-col space-y-1 w-full">
        {Object.keys(COMPONENT_GROUPS).map((groupName) => {
          ...
          const groupHasOverrides = items.some((item) => hasOverrides(item));
            ...
            <div key={`group-${groupName}`} className="flex flex-col w-full">
              ...
              {!isCollapsed && (
                <div className="flex flex-col">
                  {items.map((itemKey) => {
                    const isSelected = selectedComponent === itemKey;
                    const itemHasOverrides = hasOverrides(itemKey);
