...
const TokenColorPickerPane = ({
  ...
  function getResolvedColor(tokenName) {
    try {
      ...
      if (themeData) {
        const baseToken = tokenName.replace(/^(hover-|placeholder-)/, "");
          ...
          >
            ...
            <div className="flex flex-row flex-wrap gap-1">
              {themeVariants.map((variant) => {
                ...
                const resolvedClass = getResolvedColor(tokenName);
                ...
                return (
                  <div
                    ...
                    onMouseEnter={() => handlePreview(tokenName)}
                    title={tokenName}
                  >
                    <div
                    ...
                    <span
                      className={`text-xs leading-none ${
                        isSelected ? "text-blue-400 font-bold" : "text-gray-500"
                      }`}
                    >
                      {variant.replace("very-", "v-").substring(0, 4)}
                    </span>
                  </div>
                );
              })}
