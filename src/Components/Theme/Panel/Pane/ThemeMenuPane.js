...
import { DashPanel } from "@trops/dash-react";
                  ...
                  >
                    <div className="flex flex-col h-fit">
                      <span className="text-sm font-bold text-gray-300">
                        {capitalizeFirstLetter(colorType)}
                      </span>
                      {/* {cModel && (
                    ...
                    <ColorTile
                      ...
                      panelType="main"
                      shade={500}
                      onClick={(c) => handleSelectColor(c, colorType)}
                    />
                  </div>
                );
              })}
                ...
                >
                  ...
                  <div className="flex flex-col p-2">
                    {themeVariants
                      .filter((v) =>
                        selectedColor !== null
                          ? selectedColor["color"]["level"] === v
                          : true,
                      )
                      .map((colorLevelName) => {
                        const stringToCheck = `bg-${colorType}-${colorLevelName}`;
                        // const bgColor = theme[themeVariant][colorType];
                        const themeColor = theme[themeVariant][stringToCheck];
                        ...
                        const selected =
                          selectedColor !== null
                            ? colorType ===
                                selectedColor["color"]["colorType"] &&
                              colorLevelName ===
                                selectedColor["color"]["level"] &&
                              selectedColor["color"]["panelType"] === "sub"
                            : false;
                        return (
                          <div
                            key={colorLevelName}
                            className={`flex flex-row justify-between py-1 items-center ${
                              selectedColor === null
