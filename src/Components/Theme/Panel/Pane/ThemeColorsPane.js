...
import ThemeColorMenuItem from "../../../../Components/Theme/Panel/MenuItem/ThemeColorMenuItem";

import { colorTypes, themeVariants, objectTypes } from "@trops/dash-react";
            ...
            .map((colorType) =>
              themeVariants
                .filter((c) => {
                  ...
                  const colorToCheck =
                    themePreview[`${objectType}-${colorType}-${c}`];
                  return searchColorTerm !== "" && stringToCheck
                    ? stringToCheck.indexOf(searchColorTerm.toLowerCase()) >
                        -1 ||
                        colorToCheck.indexOf(searchColorTerm.toLowerCase()) > -1
                    : true;
                })
                .map((variant) => {
                  const stringToCheck = `${objectType}-${colorType}-${variant}`;
                  const themeToEditSelected =
                    themeEdit !== null
                      ? `${themeEdit["objectType"]}-${themeEdit["colorType"]}-${themeEdit["variant"]}`
                      : null;
                  const selected =
                    themeToEditSelected !== null
                      ? stringToCheck === themeToEditSelected
                      : true;
                  return (
                    <ThemeColorMenuItem
                      colorType={colorType}
