...
const ThemeColorMenuItem = ({
  ...
  switch (objectType) {
    ...
    case "hover-text":
      style = `${theme[`hover-text-${colorType}-${variant}`]} text-gray-900`;
      break;
  ...
  return (
    <div
      ...
      className={`cursor-pointer flex flex-col text-base font-bold rounded ${style} ${
        selected === true ? "opacity-100" : "opacity-90 hover:opacity-100"
      } text-black p-2 h-fit flex-grow`}
    >
      <div className="text-sm w-full justify-start h-full">
        {objectType}-{colorType}-{variant}
      </div>
      <div className={`flex flex-row text-xs w-full justify-start font-normal`}>
        {theme[`${objectType}-${colorType}-${variant}`]} ({backgroundColor}{" "}
        {objectType}) [{style}]
      </div>
    </div>
  );
