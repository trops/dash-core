...
export const MergeCellsModal = ({
  ...
  const handleConfirm = () => {
    ...
    if (keepOption === "first" && conflictingComponents.length > 0) {
      ...
      componentToKeep = conflictingComponents[conflictingComponents.length - 1];
    }
  ...
  const renderMergePreview = () => {
    if (cellNumbers.length === 0) {
      return (
        <div className="text-gray-400 text-center p-8">No cells selected</div>
      );
    }
        ...
        >
          <div className="text-center">
            ...
            <div className="text-gray-500 text-xs mt-1">
              {rowSpan} row{rowSpan > 1 ? "s" : ""} × {colSpan} column
              {colSpan > 1 ? "s" : ""}
            </div>
          </div>
        </div>
        <Paragraph className="text-xs text-gray-400 text-center">
          Cells {cellNumbers.join(", ")} will be merged into a single cell
        </Paragraph>
      </div>
    );
  };

  const hasConflict = conflictingComponents && conflictingComponents.length > 1;
          ...
          <div className="flex flex-col space-y-6">
              ...
              <div className="flex flex-col space-y-4">
                <div className="bg-yellow-900 border border-yellow-700 rounded p-3 text-yellow-200 text-sm">
                  <FontAwesomeIcon
                    ...
                    className="mr-2"
                  />
                  <strong>Conflict Detected:</strong> Multiple cells contain
                  widgets. Choose which widget to keep in the merged cell.
                </div>

                <div className="flex flex-col space-y-2">
                  <SubHeading3>Widget Conflict Resolution</SubHeading3>

                  <div className="flex flex-col space-y-2">
                    {/* Keep first option */}
                    <button
                      onClick={() => setKeepOption("first")}
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <div className="text-gray-200">Keep first widget</div>
                        <div className="text-xs text-gray-400">
                          Widget from cell {cellNumbers[0]}
                        </div>
                      </div>
                    </button>
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <div className="text-gray-200">Keep last widget</div>
                        <div className="text-xs text-gray-400">
                          Widget from cell {cellNumbers[cellNumbers.length - 1]}
                        </div>
                      </div>
                    </button>
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <div className="text-gray-200">Remove all widgets</div>
                        <div className="text-xs text-gray-400">
            ...
            {!hasConflict && conflictingComponents.length === 1 && (
              <div className="bg-blue-900 border border-blue-700 rounded p-3 text-blue-200 text-sm">
                ...
                One widget found. It will be kept in the merged cell.
              </div>
            )}
            ...
            {!hasConflict && conflictingComponents.length === 0 && (
              <div className="bg-gray-800 border border-gray-700 rounded p-3 text-gray-300 text-sm">
                <FontAwesomeIcon icon="info-circle" className="mr-2" />
                No widgets found in selected cells. The merged cell will be
                empty.
              </div>
            )}
          </div>
