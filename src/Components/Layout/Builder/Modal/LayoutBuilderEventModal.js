...
import { Button, FontAwesomeIcon, Panel, Modal } from "@trops/dash-react";
...
export const LayoutBuilderEventModal = ({
  ...
  const [componentsSelected] = useState({});
                  ...
                  .map((handler) => {
                    const selected =
                      eventHandlerSelected !== null
                        ? eventHandlerSelected === handler
                        : false; //isHandlerSelected(handler);
                    return (
                      <div
                        onClick={() =>
                          selected
                            ? handleRemoveEventHandler(handler)
                            : handleSelectEventHandler(handler)
                        }
                        className={`flex flex-row ${
                          selected === false && "hover:bg-gray-800"
                        } rounded cursor-pointer p-2 font-bold items-center space-x-2 ${
                          selected === true && "bg-indigo-700"
                        }`}
                      >
                  ...
                  {itemSelected !== null && (
                    <div className="flex flex-col border border-blue-800 rounded p-4 py-10 space-y-4">
                      ...
                      <p className="text-xl font-normal text-gray-300">
                        Widgets and Workspaces can talk, but we have to setup
                        the phone wires.
                      </p>
                      <p className="text-xl font-normal text-gray-300">
                        Select the method to handle the message first, then
                        select the message it will handle.
                      </p>
                    </div>
                  )}
                </div>
