...
export const LayoutItemEditHeader = ({
  ...
  function renderEditFooter(item) {
    ...
    const parentDirection = parentLayout ? parentLayout["direction"] : "col";
    ...
    const textColor = isContainer === true ? "text-gray-300" : "text-gray-300";

    console.log("HEADER ", isContainer, textColor, item, layoutItem);

    return (
      <div className={`flex flex-row space-x-1 justify-between w-full pb-1`}>
        <div className={`flex flex-row space-x-1 ${textColor}`}>
          ...
          <ButtonIcon3
            icon={`${
              direction === "col" ? "arrows-left-right" : "arrows-up-down"
            }`}
          ...
          {order > 1 && numChildren > 1 && isMinOrder === false && (
            <ButtonIcon3
              icon={`${parentDirection === "col" ? "arrow-up" : "arrow-left"}`}
              iconSize="h-3 w-3"
          ...
          {order > 1 && numChildren > 1 && isMaxOrder === false && (
            <ButtonIcon3
              icon={`${
                parentDirection === "col" ? "arrow-down" : "arrow-right"
              }`}
              iconSize="h-3 w-3"
