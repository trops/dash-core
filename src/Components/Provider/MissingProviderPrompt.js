      ...
      <Panel className="max-w-md w-full bg-white rounded-lg shadow-lg border border-yellow-200 bg-yellow-50">
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-gray-700 mt-2">
              ...
              <span className="font-semibold">{missingProviders.length}</span>{" "}
              external provider
              {missingProviders.length !== 1 ? "s" : ""} to function.
            </p>
          </div>
        </div>
            ...
            >
              <div>
                ...
                <p className="text-xs text-gray-500">
                  {Object.keys(provider.credentialSchema || {}).length} field
                  {Object.keys(provider.credentialSchema || {}).length !== 1
                    ? "s"
                    : ""}
                </p>
              </div>
              <button
                onClick={() =>
                  openSelector(provider.type, provider.credentialSchema)
                }
        ...
        <div className="mt-6 p-3 bg-blue-50 border border-blue-200 rounded">
          <p className="text-sm text-blue-900">
            💡 <span className="font-medium">Tip:</span> You can select an
            existing provider or create a new one.
          </p>
        </div>
      </Panel>
