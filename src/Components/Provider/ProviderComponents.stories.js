      ...
      >
        <ProviderForm
          ...
          onSubmit={(credentials) => {
            setMessage(
              `Provider created with credentials: ${JSON.stringify(
                credentials,
              )}`,
            );
          }}
    ...
    <div style={{ backgroundColor: "#f5f5f5", padding: "20px" }}>
      ...
      >
        <ProviderForm
          ...
          onSubmit={(credentials) => {
            setMessage(
              `Provider updated with credentials: ${JSON.stringify(
                credentials,
              )}`,
            );
          }}
  ...
  return (
    ...
    >
      ...
      {step === 3 && (
        ...
        >
          <h3 style={{ color: "#2e7d32", marginTop: 0 }}>✓ Setup Complete!</h3>
          <p style={{ color: "#1b5e20" }}>
            Provider selected: <strong>{selectedProvider}</strong>
          </p>
          <p style={{ color: "#1b5e20" }}>Your widget is now ready to use.</p>
          <button
            onClick={() => setStep(1)}
            style={{
              padding: "10px 20px",
