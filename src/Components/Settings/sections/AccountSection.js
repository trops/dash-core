...
const AuthenticatedView = ({ profile, onSignOut, onProfileUpdated }) => {
    ...
    useEffect(() => {
        ...
        async function loadPackages() {
            try {
                const result =
                    await window.mainApi?.registryAuth?.getPackages();
        ...
        <div className="flex flex-col space-y-6">
            ...
            <PackagesList
                ...
                onPackageUpdated={(updatedPkg) => {
                    setPackages((prev) =>
                        prev.map((p) =>
                            p.name === updatedPkg.name &&
                            p.scope === updatedPkg.scope
                                ? { ...p, ...updatedPkg }
                                : p,
                        ),
                    );
...
const ProfileCard = ({ profile, onSignOut, onProfileUpdated }) => {
    ...
    async function handleSave() {
        ...
        try {
            const result =
                await window.mainApi?.registryAuth?.updateProfile({
                    displayName,
                });
            if (result) {
                setEditing(false);
...
const PackageItem = ({ pkg, isEditing, onEdit, onCancel, onSaved, onDeleted }) => {
    ...
    async function handleSave() {
        ...
        try {
            ...
            const result =
                await window.mainApi?.registryAuth?.updatePackage(
                    pkg.scope,
                    pkg.name,
                    updates,
                );
