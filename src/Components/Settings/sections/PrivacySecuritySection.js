 ...
 * Privacy & Security
 *
 * Slice-2 audit panel for per-widget MCP grants. Lists every widget that
 * either declared `dash.permissions.mcp` in its package.json or has a
 * persisted user grant, with a side-by-side view of declared vs granted
 * and one-click revoke. Read-only otherwise — adding new grants happens
 * through the install consent modal, not here.
 *
 * Reads `window.mainApi.widgetMcp.{listAll,revoke,revokeServer}` exposed
 * by dash-electron's preload (see dash-electron/public/preload.js).
 */
    ...
    <div className="flex flex-col space-y-6 p-6">
      ...
      {rows.length === 0 && (
        <div className="text-sm opacity-60">
          No widgets have requested MCP permissions yet.
        </div>
      )}

      {rows.map(({ widgetId, declared, granted }) => (
        <WidgetGrantRow
          key={widgetId}
          widgetId={widgetId}
          declared={declared}
    ...
    <div className="flex flex-col space-y-3 border border-gray-700 rounded p-3">
      ...
      {!declared && !granted && (
        <span className="text-xs opacity-50">
          (no manifest, no grant — should not happen)
        </span>
      )}
