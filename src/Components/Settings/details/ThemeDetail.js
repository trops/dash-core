...
const ComponentStyleRow = ({ objectKey, styles }) => {
  ...
  const nonColorEntries = NON_COLOR_KEYS.filter((k) => styles[k]).map((k) => ({
    key: k,
    value: styles[k],
  }));
          ...
          if (key.toLowerCase().includes("text")) {
            return (
              <div
                ...
                title={`${key}: ${value}`}
              >
                <span className={`text-[8px] font-bold ${value}`}>Aa</span>
              </div>
            );
          }
        ...
        <div className="flex flex-col space-y-2">
          ...
          <Accordion2 type="multiple">
            {COMPONENT_GROUPS.map((group) => {
              ...
              const validKeys = group.keys.filter((k) => themeObjects[k]);
              if (validKeys.length === 0) return null;

              return (
                <Accordion2.Item key={group.label} value={group.label}>
                  ...
                  <Accordion2.Content value={group.label}>
                    <div className="flex flex-col">
                      {validKeys.map((k) => {
                        const objectValue = themeObjects[k];
                        return (
                          <ComponentStyleRow
                            key={k}
