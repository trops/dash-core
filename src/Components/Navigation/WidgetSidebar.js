    ...
    return (
      <div className="flex flex-col h-full">
        <div className="flex-1 overflow-y-auto px-3 py-2">
          ...
          >
            <FontAwesomeIcon icon="arrow-left" className="h-2.5 w-2.5" />
            Back
          </button>
        ...
        <RegistryAuthModal
          isOpen={registry.needsAuth}
          setIsOpen={(open) => {
            if (!open) registry.clearNeedsAuth();
          }}
          onAuthenticated={() => {
  ...
  if (registry.error) {
    return (
      <div className="flex flex-col items-center justify-center py-8 px-4 text-center">
        <span className="text-xs text-red-400 mb-2">{registry.error}</span>
        <button
          type="button"
  ...
  if (registry.packages.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-8 px-4 text-center opacity-50">
        <FontAwesomeIcon icon="compass" className="h-5 w-5 mb-2 opacity-40" />
  ...
  const uniqueAuthors = useMemo(
    () =>
      [
        ...new Set(
          allWidgets.map(
            ({ widget }) => widget.package || widget.author || "Other",
          ),
        ),
      ].sort(),
    [allWidgets],
        ...
        <div className="flex flex-col gap-2 px-3 pb-2 flex-shrink-0">
          ...
          <div className="grid grid-cols-2 gap-1.5">
            ...
            <select
              ...
              onChange={(e) => setFilterHasHandlers(e.target.value)}
              className={selectClassName}
            >
      ...
      <Sidebar.Content>
        {collapsed ? (
          <>
            <Sidebar.Item
              icon={
                <FontAwesomeIcon icon="puzzle-piece" className="h-3.5 w-3.5" />
              }
              onClick={() => {
                setActiveTab("installed");
            ...
            <Sidebar.Item
              icon={<FontAwesomeIcon icon="compass" className="h-3.5 w-3.5" />}
              onClick={() => {
          ...
          authorNames.length > 0 ? (
            authorNames.map((author) => (
              <Sidebar.Group key={author} label={author}>
                <div className="flex flex-col gap-1">
                  {filteredGrouped[author].map(({ key, widget }) => (
                    <DraggableWidgetItem
                      key={key}
                      widgetKey={key}
                      widget={widget}
      ...
      {!collapsed && activeTab === "installed" && installSuccess && (
        <Sidebar.Footer>
          <div className="flex items-center gap-2 text-xs text-emerald-400 px-1 py-1">
            <FontAwesomeIcon icon="circle-check" className="h-3 w-3" />
            {installSuccess}
          </div>
        </Sidebar.Footer>
      )}
