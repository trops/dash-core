      ...
      {showDashboards && (
        <CommandPalette.Group label="Dashboards">
          {dashboardItems.map((item) => (
            <CommandPalette.Item
              key={item.key}
              icon={<FontAwesomeIcon icon="clone" className="h-3.5 w-3.5" />}
              onSelect={() => handleSelect(item.action)}
            >
              {item.label}
              {item.isOpen && (
                <span className="text-xs opacity-40 ml-2">open</span>
              )}
              <span className="text-xs opacity-30 ml-2">{item.detail}</span>
            </CommandPalette.Item>
          ))}
      ...
      {showProviders && (
        <CommandPalette.Group label="Providers">
          {providerList.length === 0 && !query && (
            ...
            >
              <span className="opacity-40">No providers configured</span>
            </CommandPalette.Item>
          )}
          {providerList.map((item) => (
            ...
            >
              {item.label}
              {item.type && (
                <span className="text-xs opacity-40 ml-2">{item.type}</span>
              )}
            </CommandPalette.Item>
          ))}
        ...
        <CommandPalette.Group label="Themes">
          {themeList.map((item) => (
            ...
            >
              {item.label}
              {item.isActive && (
                <span className="text-xs opacity-40 ml-2">active</span>
              )}
            </CommandPalette.Item>
          ))}
