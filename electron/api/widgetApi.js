...
const widgetApi = {
  ...
  loadFolder: async (folderPath) => {
    try {
      ...
      console.log(`[WidgetApi] ✓ Loaded ${results.length} widgets from folder`);
      return results;
    } catch (error) {
      console.error(`[WidgetApi] Error loading widgets from folder:`, error);
      throw error;
    }
  },
  ...
  uninstall: async (widgetName) => {
    try {
      ...
      const success = await ipcRenderer.invoke("widget:uninstall", widgetName);
      if (success) {
  ...
  setStoragePath: async (customPath) => {
    try {
      ...
      if (result.success) {
        console.log(`[WidgetApi] ✓ Storage path changed to: ${customPath}`);
      }
      return result;
    } catch (error) {
