  ...
  getAnalyticsForQuery: (win, application, indexName, query) => {
    try {
      ...
      axios
        ...
        .then((resp) => {
          if (resp.status === 200) {
            win.webContents.send(events.ALGOLIA_ANALYTICS_FOR_QUERY_COMPLETE, {
              result: resp.data,
              indexName: indexName,
              query: query,
            });
    ...
    try {
      if (
        ...
        a.browseObjects(query, (hits) => {
          win.webContents.send(events.ALGOLIA_BROWSE_OBJECTS_UPDATE, hits);
          ...
          .catch((e) => {
            win.webContents.send(events.ALGOLIA_BROWSE_OBJECTS_ERROR, e);
          });
      } else {
        win.webContents.send(
  ...
  async partialUpdateObjectsFromDirectory(
    ...
    try {
      ...
      a.partialUpdateObjectsFromDirectorySync(
        ...
        .catch((e) => {
          win.webContents.send(events.ALGOLIA_PARTIAL_UPDATE_OBJECTS_ERROR, e);
        });
    } catch (e) {
  ...
  createBatchesFromFile: (
    ...
    try {
      ...
      a.createBatchesFromJSONFile(
        ...
        .then((result) => {
          win.webContents.send(events.ALGOLIA_CREATE_BATCH_COMPLETE, result);
        })
        .catch((e) => {
